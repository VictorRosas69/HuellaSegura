    ...
    <div className="min-h-screen pb-24" style={{ background: '#0F0F1A' }}>
      ...
      <div className="px-5 pt-safe pt-5 pb-4"
        ...
        <div className="flex items-center justify-between mb-1">
          <h1 className="font-poppins font-bold text-2xl text-white">Alertas</h1>
          {noLeidas > 0 && (
      ...
      <div className="px-5 py-3">
        <div className="flex items-center gap-2">
          {TABS.map((t, i) => (
            <motion.button key={t.label} whileTap={{ scale: 0.95 }} onClick={() => setTab(i)}
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setShowFiltro(true)}
            ...
            style={{ background: filtrosActivos > 0 ? 'linear-gradient(135deg,#FF9280,#F97B62)' : 'rgba(255,255,255,0.07)', border: filtrosActivos === 0 ? '1px solid rgba(255,255,255,0.08)' : 'none' }}>
            <SlidersHorizontal size={16} color={filtrosActivos > 0 ? 'white' : 'rgba(255,255,255,0.4)'} />
            {filtrosActivos > 0 && (
        ...
        {!loading && notifsFiltradas.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="h-20 w-20 rounded-3xl flex items-center justify-center"
                 style={{ background: 'rgba(249,123,98,0.1)', border: '1px solid rgba(249,123,98,0.2)' }}>
              <Bell size={32} style={{ color: 'rgba(249,123,98,0.5)' }} strokeWidth={1.5} />
            </div>
            <div>
              <p className="font-poppins font-bold text-base text-white">Sin alertas</p>
        ...
        <AnimatePresence>
          {!loading && notifsFiltradas.map((notif, i) => (
            <motion.div
              ...
              style={{
                background: notif.leida ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.06)',
                border: notif.leida ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(249,123,98,0.2)',
                opacity: notif.leida ? 0.65 : 1,
              }}>
              <div className="flex items-start gap-3">
                <div className="h-11 w-11 rounded-xl flex items-center justify-center shrink-0"
                     style={{ background: notif.tipo === 'avistamiento' ? 'rgba(0,196,180,0.15)' : 'rgba(249,123,98,0.15)', border: `1px solid ${notif.tipo === 'avistamiento' ? 'rgba(0,196,180,0.25)' : 'rgba(249,123,98,0.25)'}` }}>
                  {notif.tipo === 'avistamiento'
                    ? <MapPin size={18} style={{ color: '#00C4B4' }} strokeWidth={2} />
                    : <Bell   size={18} style={{ color: '#F97B62' }} strokeWidth={2} />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm leading-snug line-clamp-3 text-white"
                     style={{ fontWeight: notif.leida ? 400 : 600 }}>
                    {notif.mensaje}
                  </p>
        ...
        {showFiltro && (
          <>
            ...
            <motion.div
              ...
              style={{ background: '#1A1A2E', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 -8px 40px rgba(0,0,0,0.5)' }}>
              <div className="flex justify-center mb-4">
                <div className="h-1 w-10 rounded-full" style={{ background: 'rgba(255,255,255,0.2)' }} />
              </div>
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-poppins font-bold text-lg text-white">Filtrar alertas</h2>
              ...
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(255,255,255,0.35)' }}>Tipo</p>
              ...
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(255,255,255,0.35)' }}>Estado</p>
